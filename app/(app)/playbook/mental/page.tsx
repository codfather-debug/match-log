...
function PillarCard({ p }: { p: typeof PILLARS[0] }) {
  ...
  return (
    <div className={`rounded-2xl border transition-all ${open ? p.color : 'border-zinc-800 bg-zinc-900/50'}`}>
      <button
        onClick={() => setOpen(v => !v)}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`text-sm font-bold ${open ? p.badge : 'text-zinc-100'}`}>{p.label}</span>
        </div>
        <svg
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-zinc-800 pt-3">
          <p className="text-sm text-zinc-400 italic">{p.summary}</p>
          ...
          <div className={`rounded-xl border p-3 mt-1 ${p.color}`}>
            ...
            <p className="text-sm text-zinc-300">{p.cue}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="space-y-7">
        ...
        <section className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.04] p-4 text-center space-y-1">
          ...
          <p className="text-sm text-zinc-400">Technical skill gets you on court. Mental skill wins matches.</p>
        </section>

        {/* 4 Pillars */}
        <section className="space-y-3">
          <div>
            <p className="text-xs font-black tracking-widest uppercase text-zinc-400">4 Mental Pillars</p>
            <p className="text-xs text-zinc-500 mt-0.5">The same metrics tracked in your match reflections</p>
          </div>
        ...
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Between-Point Routine</p>
          <p className="text-xs text-zinc-500">Do this after every single point — win or lose</p>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden divide-y divide-zinc-800">
            {BETWEEN_POINT.map(b => (
              <div key={b.step} className="flex items-start gap-4 px-4 py-3.5">
                ...
                <div>
                  <p className="text-sm font-bold text-zinc-200">{b.label}</p>
                  <p className="text-xs text-zinc-400 mt-0.5">{b.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Momentum Management */}
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Momentum Management</p>
          <div className="space-y-2">
            {MOMENTUM.map(m => (
              <div key={m.situation} className={`rounded-2xl border p-4 space-y-1.5 ${m.color} bg-zinc-900/50`}>
                <p className={`text-xs font-black uppercase tracking-wider ${m.color.split(' ')[1]}`}>{m.situation}</p>
                <p className="text-sm text-zinc-400">{m.action}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Language Protocol */}
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Self-Talk Protocol</p>
          <p className="text-xs text-zinc-500">Replace negative self-talk in real time</p>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden divide-y divide-zinc-800">
            {LANGUAGE.map((l, i) => (
        ...
        {/* Body Language */}
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Body Language</p>
          <p className="text-xs text-zinc-500">Your body broadcasts your mental state — to yourself and your opponent</p>
        ...
        {/* Pre-Match Prep */}
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Pre-Match Prep</p>
          <div className="space-y-2">
            {PRE_MATCH.map((p, i) => (
              <div key={i} className="flex items-start gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                <span className="text-lg flex-shrink-0">{p.icon}</span>
                <p className="text-sm text-zinc-400">{p.tip}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-3">
          <p className="text-xs font-black tracking-widest uppercase text-zinc-400">Common Mental Mistakes</p>
          <div className="space-y-2">
            {MISTAKES.map((m, i) => (
              <div key={i} className="rounded-2xl border border-red-500/15 bg-red-500/[0.03] p-4 space-y-1.5">
                ...
                <p className="text-sm text-zinc-400">{m.fix}</p>
              </div>
            ))}
          </div>
        </section>
